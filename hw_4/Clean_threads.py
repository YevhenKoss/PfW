...
#Функция создания списка файлов с известным расширением
		...
		if element.is_file():
			for key, val in files.items():
				if element.suffix in val:
					...
					data.append(element)
					data.append(new_file_path)
	return data

#Функция перемещения файлов с известным расширением
def replace_known_files(element, new_file_path):
...
def main():
	...
	for i in range(0, max_range):
		ts.append(Thread(target=replace_known_files, args=(data[2*i], data[2*i+1])))
		ts[-1].start()
	...
	print()
	print("Времени затрачено: {:.8f}s".format(elapsed))
